...
public class Level4 : MonoBehaviour
{
    ...
    void MakeRain()
    {
        int rand = Random.Range(0, 9); //파란비:검정비:황사비 = 5:3:1
    ...
    void MakeItem()
    {
        int rand = Random.Range(0, 10); //햄버거:아몬드:소주:발모제 = 4:3:3:1

        if(rand == 6)
        {
            Instantiate(Resources.Load("Prefabs/2D/Balmo"), rainSpawns[Random.Range(0, 6)]);
        }else if (rand == 0 || rand == 1 || rand == 5)
    ...
    void MakeUmbrella()
    {
        int rand = Random.Range(0, 3); //일반우산:찢어진우산 = 1:2
        Object umbrella;
        if (rand == 1)
        {
